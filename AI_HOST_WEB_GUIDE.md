...
- `固件烧录`: Auxiliary serial flashing. The write target is a fixed select with six choices: `0x0`, `0x8000`, `0xf000`, `0x20000`, `0x620000`, and both app slots. The target only determines write offsets; do not hard-bind file names to targets. Online firmware uses `app.url`, and custom `.bin` files may be written to any target at the user's discretion.
...
- Firmware flashing must not read GitHub Release, scan GitHub repository files, or directly access the private R2 Bucket. It prioritizes `https://rlcd-update.wickenzh.workers.dev/firmware/versions.json`, displays the recent `items`, and verifies downloaded bytes with Web Crypto before enabling flashing. For online firmware, download `app.url` and let the selected write target determine the offset. If size or SHA-256 verification fails, clear the downloaded in-memory data and require the user to download again. If `versions.json` is temporarily unavailable, the app may fall back to `latest.json` to display and download the latest OTA app package.
