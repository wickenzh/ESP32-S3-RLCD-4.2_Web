...
- `固件烧录`: Auxiliary serial flashing. The write target is a fixed select with six choices: `0x0` bootloader only, `0x8000` partition table only, `0xf000` ota_data only, `0x20000` app to `ota_0`, `0x620000` app to `ota_1`, and both app slots. Online firmware uses `app.url` only for app targets; bootloader, partition table, and ota_data targets require matching local files.
...
- Firmware flashing must not read GitHub Release, scan GitHub repository files, or directly access the private R2 Bucket. It prioritizes `https://rlcd-update.wickenzh.workers.dev/firmware/versions.json`, displays the recent `items`, and verifies downloaded bytes with Web Crypto before enabling flashing. For online firmware, download `app.url` and allow only app targets (`ota_0`, `ota_1`, or both). If size or SHA-256 verification fails, clear the downloaded in-memory data and require the user to download again. If `versions.json` is temporarily unavailable, the app may fall back to `latest.json` to display and download the latest OTA app package for app targets only.
