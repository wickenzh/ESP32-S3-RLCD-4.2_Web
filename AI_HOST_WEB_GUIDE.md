...
- Firmware flashing must not read GitHub Release, scan GitHub repository files, or directly access the private R2 Bucket. It reads only `https://rlcd-update.wickenzh.workers.dev/firmware/versions.json`, displays the recent `items`, validates that each usable item has both `app` and `merged` entries with `url`, `sha256`, and `size`, downloads `merged.url` for serial full flashing, and verifies SHA-256 with Web Crypto before enabling flashing. If size or SHA-256 verification fails, clear the downloaded in-memory data and require the user to download again. `latest.json` is for device OTA checks, not the host web's primary list.
